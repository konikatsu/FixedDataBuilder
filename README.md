...
![FixedDataBuilder のスクリーンショット](docs/screen-image-occurs-visual-field-v0.1.32.png)
...
- OCCURS 展開項目は、項目縦表示ではグループ見出し行、レコード縦表示ではグループ見出し列と淡色背景でまとまりを表示
- レコード追加、複製、削除
- セル編集
...
項目縦表示では `明細-1` / `明細-2` のような見出し行を挟み、レコード縦表示では各 OCCURS グループの先頭に見出し列を表示します。

![OCCURS レコード縦表示のスクリーンショット](docs/screen-image-occurs-visual-record-v0.1.32.png)
